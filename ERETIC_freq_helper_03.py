...
def plot_freq_schematic(fov_centr_freq,
	...
	ax.text((base_res / 5), (base_res * 0.4), ('recommended \n ERETIC frequency: \n' +
											   str(rec_eretic_freq)), props, rotation=0)
	...
	if freq_increases_to_left == True:	
		ax2.invert_xaxis()
	else:
		pass
...
def unpack_params_and_run(scan_title, scan_toggle, scan_param_dict):
	if scan_toggle == False:
		...
		plot_freq_schematic(fov_centr_freq,
							...
							min_freq,
							max_freq,
							title = (str(scan_title) + " " + str(now_s_format)))
...
if __name__ == "__main__":
	unpack_params_and_run('3D GRE', gre_3d_toggle, gre_3d_params)
	unpack_params_and_run('MRI SE qT2', q_t2_toggle, q_t2_params)
	unpack_params_and_run('8-Point Dixon', dixon_8pt_toggle, dixon_8pt_params)
	plt.show()
